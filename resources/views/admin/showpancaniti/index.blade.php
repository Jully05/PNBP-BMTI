...
@section('content')
    <div class="container">
        <div class="bg-primary py-3">
            <h3 class="text-white text-center">JADWAL BOOKING GEDUNG PANCANITI</h3>
        </div>

        <div class="row justify-content-center mt-4">
            <div class="col-md-10 d-flex justify-content-end">
                <a href="{{ route('showpancaniti.create') }}" class="btn btn-primary">Tambah Booking +</a>
            </div>
        </div>
                ...
                <div class="card border-0 shadow-lg my-4">
                    ...
                    <div class="card-body">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($tableBookingsPancaniti as $key => $booking)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        ...
                                        <td>
                                            <a href="{{ route('showpancaniti.edit', $booking) }}" class="btn btn-dark"><i
                                                    class="fas fa-pen"></i></a>
                                            <a href="#" onclick="confirmDelete({{ $booking->id }}, event);"
                                                class="btn btn-danger"><i class="fas fa-trash-alt"></i></a>
                                            <form id="delete-product-from-{{ $booking->id }}"
                                                action="{{ route('showpancaniti.destroy', $booking->id) }}" method="post">
                                                @csrf
                                                @method('delete')
                                            </form>
                                        </td>
